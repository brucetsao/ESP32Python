...
relay = Pin(5, Pin.OUT)                   # 創建一個控制 GPIO5 的 Pin 物件作為輸出
relay.value(0)                            # 將 繼電器模組 的初始狀態設為關閉 (0)

gpio_state = 'OFF'                      # 設定 繼電器模組 的狀態為關閉

# 定義一個返回 HTML 網頁的函數
def web_page():
    if relay.value() == 1:                # 檢查 LED 的狀態
        gpio_state = 'ON'               # 如果 繼電器模組 開啟，設為 'ON'
    else:
        gpio_state = 'OFF'              # 如果 繼電器模組 關閉，設為 'OFF'
    ...
    <html>
        ...
        <body>
            ...
            <p>Relay Status: """ + gpio_state + """</p>  <!-- 顯示當前 LED 的狀態 -->
            <a href="/?device=on"><button class="button">ON</button></a>  <!-- LED 開啟按鈕 -->
            <a href="/?device=off"><button class="button button2">OFF</button></a>  <!-- LED 關閉按鈕 -->
        </body>
        </html>
    """
...
while True:	#永久迴圈，使其網頁在一直等待再被連接狀態
    ...
    device_on = request.find('/?device=on')   # 查找是否有開啟 繼電器模組 的請求
    device_off = request.find('/?device=off') # 查找是否有關閉 繼電器模組 的請求

    if device_on == 6:                     # 如果請求中包含 '/?device=on'
        relay.value(1)                    # 開啟 繼電器模組
    if device_off == 6:                    # 如果請求中包含 '/?device=off'
        relay.value(0)                    # 關閉 繼電器模組

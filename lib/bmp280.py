...
# 作者: David Stenwall (david at stenwall.io)

# 電源模式
BMP280_POWER_SLEEP = const(0)  # 睡眠模式
BMP280_POWER_FORCED = const(1)  # 強制模式
BMP280_POWER_NORMAL = const(3)  # 正常模式
...
# 使用案例
BMP280_CASE_HANDHELD_LOW = const(0)  # 手持低功耗
BMP280_CASE_HANDHELD_DYN = const(1)  # 手持動態
BMP280_CASE_WEATHER = const(2)  # 氣象監測
BMP280_CASE_FLOOR = const(3)  # 地板測量
BMP280_CASE_DROP = const(4)  # 掉落測量
BMP280_CASE_INDOOR = const(5)  # 室內測量
...
_BMP280_REGISTER_CONFIG = const(0xF5)  # IIR 濾波器配置
...
class BMP280:
    def __init__(self, i2c_bus, addr=0x76, use_case=BMP280_CASE_HANDHELD_DYN):
        ...
        self.read_wait_ms = 0  # 強制測量和讀數之間的間隔
        self._new_read_ms = 200  # 讀數之間的間隔

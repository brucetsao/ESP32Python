...
# 並將其數據顯示在 SSD1306 OLED 顯示模組上。
# 這個程式碼不斷從 HTU21D 感測器取得溫度和濕度數據，
# 並將其顯示在 SSD1306 OLED 顯示模組上。
# 顯示內容包括溫濕度數據以及 MAC 位址。程式還會在控制台輸出這些資訊。
...
display.show()  # 更新 OLED 顯示模組，清除之前的內容

# 在 OLED 顯示模組上顯示 MAC 位址
display.text(GetMAC(), 0, 0, 1)  # 在位置 (0, 0) 顯示 MAC 位址
